...
def generate_gaussian_noise_image(image_size=(224, 224), mean=0, std=50):
    """生成高斯噪声图片并保存"""
    noise = np.random.normal(mean, std, (*image_size, 3)).clip(0, 255).astype(np.uint8)
    noise_image = Image.fromarray(noise)
    noise_image.save("/hpc2hdd/home/zrao690/MultimodalEmb/data/beauty/handled/noise.jpg")
    return noise_image

class QwenVLTrainMask(object):
    
    def __init__(self, data_args, model_args, training_args, processor: Qwen2_5_VLProcessor) -> None:
    ...
    def multimodal_mask(self, examples):
        ...
        for i in range(len(examples[self.attributes_input_column])):
            if examples[self.attributes_input_column][i]:
                ...
                try:
                    image_sample_input_ids = self.processor.apply_chat_template(constrastive_sample_input_template, 
                ...
                model_inputs["contrastive_sample_input"].append(image_sample_input_ids)
        return model_inputs
            ...
            INPUT_TEMPLATE = [
                {
                    "role":"user",
                    "content":[
                        {"type":"image", "image": f"{image}"},#, "resized_height": 100, "resized_width": 100},
        ...
        for i in range(len(examples[self.attributes_input_column])):
            if examples[self.attributes_input_column][i]:
                ...
                if self.enable_moelora and self.enable_task_gate:
                    moe_task_ids = [0, 1]
                    model_inputs["moe_task_ids"].append(moe_task_ids)
                model_inputs["sample_input"].append(image_sample_input_ids)
        return model_inputs
