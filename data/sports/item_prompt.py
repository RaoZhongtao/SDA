...
instruction = "The sports item has the following attributes: \n "
...
for key, value in item_data.items():
    json_data.append({"attributes_input": value, "target": "", "item": key, "item_id": int(id_map[key])})
# 按照 item_id 进行递增排序
json_data.sort(key=lambda x: x["item_id"])
save_data("text_item_str_debug.jsonline", json_data)