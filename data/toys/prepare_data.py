...
output_filename = "multimodal_item_str"
# multimodal_item_str.jsonline
data = json.load(open("./handled/item2attributes.json", "r"))


instruction_head = "The beauty product has the following attributes: "

# instruction_end = ". Summary above product in one word:"
...
for item_dict in tqdm(data.values()):
    item_prompt = copy.deepcopy(instruction_head)
    item_id = None
    # item_title = None
    ...
    if item_id:
        item_data[item_id] = item_prompt[:-2]# + instruction_end
        # if item_title:
        #     id2title[item_id] = item_title
...
save_data(f"{output_filename}.jsonline", json_data)
